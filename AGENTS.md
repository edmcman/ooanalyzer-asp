...
| `propagator/introspect.py` | Duty-cycled near-zero-idle sampler: with `--introspect FILE` writes a JSONL time-series of search internals (reward-family selection, decisions/backtracks/restarts by family, decision depth) over wall-clock time, so the merge-reward plateau can be *seen* evolving. See its docstring for the measured overhead |
| `scripts/viz_introspect.py` | Renders a `--introspect` JSONL trace into an interactive self-contained plotly HTML dashboard (anytime cost + reward/backtrack/depth trajectories); `uv run python scripts/viz_introspect.py TRACE.jsonl` |
