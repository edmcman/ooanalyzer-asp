...
| `propagator/introspect.py` | Reward-family helpers for the native `--introspect FILE` sampler; the Rust inspector writes a JSONL time-series of reward selection, every symbolic predicate's direct decisions and propagation-implied assignments, backtracks, and depth over wall-clock time |
