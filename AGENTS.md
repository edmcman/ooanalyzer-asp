...
| `propagator/introspect.py` | Reward-family helpers for the native `--introspect FILE` sampler; the Rust inspector writes a JSONL time-series of reward selection, sampled symbolic predicates' direct decisions and propagation-implied assignments, backtracks, and depth over wall-clock time. It watches a reproducible 1% subset by default; override with `--introspect-atom-sample=RATE` |
| `scripts/viz_introspect.py` | Renders a `--introspect` JSONL trace into an interactive self-contained plotly HTML dashboard (anytime cost, reward/backtrack/depth, and direct-vs-implied predicate distributions); `uv run python scripts/viz_introspect.py TRACE.jsonl` |
| `scripts/introspect2speedscope.py` | Converts a `--introspect` JSONL trace's `stack` records (per-thread, per-window decision-stack snapshots — level 1..current read straight off clingo's assignment via `ffi.decision()`, see `rust/src/inspector.rs`) into a real nested speedscope flamegraph, plus flat count-ranked profiles for `backtracks`/`decided`/`implied`/`true_now`; `uv run python scripts/introspect2speedscope.py TRACE.jsonl` |
